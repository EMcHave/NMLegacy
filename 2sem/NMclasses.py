...
class Plots:
    ...
    def build(self, scale = 'Normal'):
        ...
        for number in range(len(self.ars)): 
            ...
            if self.legend != None:
                ax.plot(self.ars[number][0], self.ars[number][1], col, label = self.legend[number])
                if scale == 'logX':
                    ax.set_xscale('log')
                if scale == 'logY':
                    ax.set_yscale('log')
                ...
                ax.plot(self.ars[number][0], self.ars[number][1], col)
                if scale == 'logX':
                    ax.set_xscale('log')
                if scale == 'logY':
                    ax.set_yscale('log')
...
class Integtration:
    ...
    def lobatto(self):
        ...
        for i in range(len(weighs)):
            integral += self.func( (self.b - self.a)*t_i[i]/2 + (self.a + self.b)/2 )*weighs[i]
